...
function cmprPlot(event, r1, ini, final, alg, side){
        ...
        var data = [  {    x: ['FCFS', 'SSTF', 'SCAN', 'C-SCAN', 'LOOK', 'C-LOOK'],
...
for (var i = 1; i < data[0].y.length; i++) {
  if (data[0].y[i] < min) {
    min = data[0].y[i];
    minX = [data[0].x[i]];
  } else if (data[0].y[i] == min) {
    minX.push(data[0].x[i]);
  }
}

var minValues = [];
var minVal = Math.min(...data[0].y);
for (var i = 0; i < data[0].y.length; i++) {
  if (data[0].y[i] == minVal) {
    minValues.push(i);
  }
}
data[0].marker = {
        color: Array(data[0].y.length).fill('#245953')
      };
        for (var i = 0; i < minValues.length; i++) {
        data[0].marker.color[minValues[i]] = '#2459534b';
      }  

document.getElementById("leastValue").innerHTML = "Minimum seek time is: " + min + ", Corresponding algorithm(s) is/are: " + minX.join(", ");   
